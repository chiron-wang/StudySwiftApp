...
class ViewController: BaseViewController {
    ...
    override func viewDidLoad() {
        ...
        // 建立 UITableView
        myTableView = UITableView(frame: CGRect(x: 0, y: 44, width: fullSize.width, height: fullSize.height - 108), style: .plain)
    ...
    func updateRecordContent(_ indexPath: IndexPath) {
        ...
        // 建立輸入框
        updateAlertController.addTextField { (textField: UITextField!) in
            textField.text = name
        ...
        // 建立[更新]按鈕
        let okAction = UIAlertAction(title: "更新", style: .default) { (action: UIAlertAction!) in
