...
class BaseMainViewController: UIViewController, CLLocationManagerDelegate, UITableViewDelegate, UITableViewDataSource, URLSessionDelegate, URLSessionDownloadDelegate {
    ...
    override func viewDidLoad() {
        ...
        self.taipeiDateUrl = "https://data.taipei/opendata/datalist/apiAccess?scope=resourceAquire&rid="
    ...
    func addData() {
        ...
        if self.todayDateInt - date > self.refreshDays {
            self.normalGet(self.taipeiDateUrl + self.strTargetID)
        } else {
    ...
    func jsonParse(_ url: URL) {
        do {
            let dict = try JSONSerialization.jsonObject(with: Data(contentsOf: url), options: .allowFragments) as! [String:[String:AnyObject]]
    ...
    func reloadAPIData() {
        guard self.apiDataAll != nil else { return }
